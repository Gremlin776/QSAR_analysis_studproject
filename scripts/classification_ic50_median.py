...
try:
    ...
    print("Ошибка: Библиотека imblearn не найдена! Установите: pip install imbalanced-learn")
    exit(1)
...
# Создание директорий для логов, графиков и результатов
try:
...
logging.basicConfig(
    filename='logs/classification_ic50_median.log',
    level=logging.INFO,
...
def main():
    """Основная функция для классификации IC50 по медиане."""
    ...
    # Проверка наличия колонки IC50_mM
    if 'IC50_mM' not in data.columns:
        logging.error("Колонка IC50_mM отсутствует")
        print("Ошибка: Колонка IC50_mM отсутствует!")
        exit(1)

    # Бинаризация IC50 по медиане
    median_ic50 = data['IC50_mM'].median()
    data['IC50_median'] = (data['IC50_mM'] >= median_ic50).astype(int)
    logging.info(f"Медиана IC50: {median_ic50:.2f}, доля положительного класса: {data['IC50_median'].mean():.2%}")

    # Выбор признаков
    features = [col for col in data.columns if col not in ['IC50_mM', 'CC50_mM', 'SI', 'log_ic50', 'log_cc50', 'log_si', 'IC50_median']]
    X = data[features]
    y = data['IC50_median']
    ...
    if X.isnull().values.any() or np.any(np.isinf(X.values)):
        ...
        exit(1)
    if y.isnull().any():
        logging.error("NaN значения в IC50_median")
        print("Ошибка: NaN значения в IC50_median!")
        exit(1)
    ...
    try:
        ...
        print(f"Ошибка масштаба: {e}")
        exit(1)

    # Применение SMOTE при дисбалансе классов
    class_counts = pd.Series(y_train).value_counts(normalize=True).to_dict()
    logging.info(f"Баланс классов: {class_counts}")
    if abs(class_counts.get(0, 0) - class_counts.get(1, 0)) > 0.1:
        logging.info("Применение SMOTE из-за дисбаланса классов")
        try:
            min_samples = min(pd.Series(y_train).value_counts().values)
            if min_samples < 5:
                logging.warning(f"Недостаточно образцов для SMOTE: {min_samples}. Пропуск SMOTE.")
                print("Предупреждение: Ошибка недостатка данных для SMOTE.")
            else:
                smote = SMOTE(random_state=42, k_neighbors=min(5, min_samples-1))
                X_train_scaled, y_train = smote.fit_resample(X_train_scaled, y_train)
                logging.info(f"После SMOTE: train samples={X_train_scaled.shape[0]}")
        except Exception as e:
            logging.error(f"Ошибка SMOTE: {e}")
            print(f"Ошибка SMOTE: {e}")
    else:
        logging.info("Классы сбалансированы, SMOTE не требуется")
    ...
    for model_class, model_name in models:
        logging.info(f"Обучающая модель {model_name}")
        ...
        if hasattr(model, 'feature_importances_'):
            try:
                ...
                plt.title(f'Важность признаков (IC50_median, {model_name})')
                plt.savefig(f'figures/feature_importance_ic50_median_{model_name.lower()}.png')
                plt.close()
                logging.info(f"Важность признаков сохранена: figures/feature_importance_ic50_median_{model_name.lower()}.png")
            except Exception as e:
    ...
    # Обучение ансамбля
    logging.info("Обучение моделирования")
    estimators = [
    ...
    if best_y_pred is not None:
        try:
            ...
            plt.title(f'Матрица ошибок (IC50_median, {best_model_name})')
            plt.ylabel('Истинный класс')
            plt.xlabel('Предсказанный класс')
            plt.savefig(f'figures/confusion_matrix_ic50_median_{best_model_name.lower()}.png')
            plt.close()
            logging.info(f"Матрица ошибок сохранена: figures/confusion_matrix_ic50_median_{best_model_name.lower()}.png")

            # ROC-кривая
            fpr, tpr, _ = roc_curve(y_test, best_y_proba)
            plt.figure(figsize=(8, 6))
            plt.plot(fpr, tpr, label=f'ROC кривая (AUC={roc_auc_score(y_test, best_y_proba):.3f})')
            plt.plot([0, 1], [0, 1], 'r--')
            plt.xlabel('Ложно-положительный уровень')
            plt.ylabel('Истинно-положительный уровень')
            plt.title(f'ROC кривая (IC50_median, {best_model_name})')
            plt.legend()
            plt.savefig(f'figures/roc_curve_ic50_median_{best_model_name.lower()}.png')
            plt.close()
            logging.info(f"ROC кривая сохранена: figures/roc_curve_ic50_median_{best_model_name.lower()}.png")
        except Exception as e:
            logging.error(f"Ошибка сохранения графиков для {best_model_name}: {e}")

    # Сохранение результатов
    try:
        results_df = pd.DataFrame(results)
        results_df = results_df.round(3)
        results_df.to_csv('results/classification_ic50_median.csv', index=False)
        logging.info("Результаты классификации IC50_median:\n" + results_df.to_string())
        print("Результаты классификации IC50_median:")
        print(results_df)
    except Exception as e:
