...
logging.basicConfig(
    filename='logs/classification_cc50_median.log',
    level=logging.INFO,
...
def main():
    """Основная функция для классификации CC50 по медиане."""
    ...
    # Проверка наличия колонки CC50_mM
    if 'CC50_mM' not in data.columns:
        logging.error("Колонка CC50_mM отсутствует")
        print("Ошибка: Колонка CC50_mM отсутствует!")
        exit(1)

    # Бинаризация CC50 по медиане
    median_cc50 = data['CC50_mM'].median()
    data['CC50_median'] = (data['CC50_mM'] >= median_cc50).astype(int)
    logging.info(f"Медиана CC50: {median_cc50:.2f}, доля положительного класса: {data['CC50_median'].mean():.2%}")

    # Выбор признаков
    features = [col for col in data.columns if col not in ['IC50_mM', 'CC50_mM', 'SI', 'log_ic50', 'log_cc50', 'log_si', 'CC50_median']]
    X = data[features]
    y = data['CC50_median']
    ...
    if X.isnull().values.any() or np.any(np.isinf(X.values)):
        ...
        exit(1)
    if y.isnull().any():
        logging.error("NaN значения в CC50_median")
        print("Ошибка: NaN значения в CC50_median!")
        exit(1)
    ...
    if abs(class_counts.get(0, 0) - class_counts.get(1, 0)) > 0.1:
        ...
        try:
            ...
            if min_samples < 5:
                ...
                print("Предупреждение: Недостаточно образцов для SMOTE.")
            else:
    ...
    for model_class, model_name in models:
        ...
        if hasattr(model, 'feature_importances_'):
            try:
                ...
                plt.title(f'Важность признаков (CC50_median, {model_name})')
                plt.savefig(f'figures/feature_importance_cc50_median_{model_name.lower()}.png')
                plt.close()
                logging.info(f"Важность признаков сохранена: figures/feature_importance_cc50_median_{model_name.lower()}.png")
            except Exception as e:
    ...
    if best_y_pred is not None:
        try:
            ...
            plt.title(f'Матрица ошибок (CC50_median, {best_model_name})')
            plt.ylabel('Истинный класс')
            plt.xlabel('Предсказанный класс')
            plt.savefig(f'figures/confusion_matrix_cc50_median_{best_model_name.lower()}.png')
            plt.close()
            logging.info(f"Матрица ошибок сохранена: figures/confusion_matrix_cc50_median_{best_model_name.lower()}.png")
            ...
            plt.title(f'ROC кривая (CC50_median, {best_model_name})')
            plt.legend()
            plt.savefig(f'figures/roc_curve_cc50_median_{best_model_name.lower()}.png')
            plt.close()
            logging.info(f"ROC кривая сохранена: figures/roc_curve_cc50_median_{best_model_name.lower()}.png")
        except Exception as e:
    ...
    try:
        ...
        results_df.to_csv('results/classification_cc50_median.csv', index=False)
        logging.info("Результаты классификации CC50_median:\n" + results_df.to_string())
        print("Результаты классификации CC50_median:")
        print(results_df)
    except Exception as e:
