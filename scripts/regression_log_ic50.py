...
# Проверяем наличие всех необходимых библиотек, чтобы обеспечить выполнение регрессии log_ic50, которая предсказывает противовирусную активность соединений
...
logging.basicConfig(
    filename='logs/regression_log_ic50.log',
    level=logging.INFO,
...
def train_model(model_class, X_train, X_test, y_train, y_test, model_name):
    ...
    """
    # Определяем функцию оптимизации гиперпараметров с использованием Optuna, чтобы минимизировать среднеквадратичную ошибку и улучшить качество предсказаний log_ic50
    def objective(trial):
        try:
            if model_name == 'RF':
                # Настраиваем параметры RandomForest для регрессии log_ic50, включая количество деревьев и ограничения на их сложность, чтобы сбалансировать точность и обобщающую способность
                params = {
    ...
    try:
        # Обучаем модель на обучающей выборке, чтобы она могла предсказывать log_ic50
        ...
        # Вычисляем метрики качества: MSE, R2 и MAE, чтобы оценить, насколько точно модель предсказывает log_ic50
...
def main():
    """Основная функция для регрессии log_ic50."""
    ...
    # Проверяем наличие колонки log_ic50, которая является целевой переменной для регрессии
    if 'log_ic50' not in data.columns:
        logging.error("Колонка log_ic50 отсутствует")
        print("Ошибка: Колонка log_ic50 отсутствует!")
        exit(1)
    ...
    if y.isnull().any() or np.any(np.isinf(y.values)):
        logging.error("NaN или бесконечные значения в log_ic50")
        print("Ошибка: NaN или бесконечные значения в log_ic50!")
        exit(1)
    ...
    for model_class, model_name in models:
        ...
        # Визуализируем важность признаков для моделей, поддерживающих эту функцию, чтобы понять, какие молекулярные дескрипторы наиболее влияют на log_ic50
        if hasattr(model, 'feature_importances_'):
            try:
                ...
                plt.title(f'Важность признаков (log_ic50, {model_name})')
                plt.savefig(f'figures/feature_importance_log_ic50_{model_name.lower()}.png')
                plt.close()
                logging.info(f"Важность признаков сохранена: figures/feature_importance_log_ic50_{model_name.lower()}.png")
            except Exception as e:
    ...
    if best_y_pred is not None:
        try:
            # Создаем график рассеяния истинных и предсказанных значений log_ic50, чтобы визуально оценить точность модели
            plt.figure(figsize=(8, 6))
            plt.scatter(y_test, best_y_pred, alpha=0.5)
            plt.plot([y_test.min(), y_test.max()], [y_test.min(), y_test.max()], 'r--')
            plt.xlabel('Истинный log_ic50')
            plt.ylabel('Предсказанный log_ic50')
            plt.title(f'Предсказания vs Истинные значения (log_ic50, {best_model_name})')
            plt.savefig(f'figures/pred_vs_true_log_ic50_{best_model_name.lower()}.png')
            plt.close()
            logging.info(f"График предсказаний сохранен: figures/pred_vs_true_log_ic50_{best_model_name.lower()}.png")
        except Exception as e:
    ...
    try:
        ...
        results_df.to_csv('results/regression_log_ic50.csv', index=False)
        logging.info("Результаты регрессии log_ic50:\n" + results_df.to_string())
        print("Результаты регрессии log_ic50:")
        print(results_df)
    except Exception as e:
...
if __name__ == "__main__":
    # Запускаем основную функцию для выполнения регрессии log_ic50
    main()